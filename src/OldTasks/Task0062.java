        ...
        for (int i = 0; i < numberOfTest; i++) {
            ...
            if (line.length() == line2.length()) {
                for (int j = 0; j < chars2.length; j++) {
                    temp = Integer.parseInt(String.valueOf(chars[j])) + Integer.parseInt(String.valueOf(chars2[j]));

                    if (temp % 2 == 0) {
                        chars[j] = Character.forDigit(temp / 2, 10);
                        chars2[j] = Character.forDigit(temp / 2, 10);
                        continue;
                    }

                    if (j % 2 != 0) {
                        chars[j] = Character.forDigit(temp / 2 + 1, 10);
                        chars2[j] = Character.forDigit(temp / 2, 10);
                    } else {
